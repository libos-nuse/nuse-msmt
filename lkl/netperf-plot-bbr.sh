...
set output "${OUTPUT}/${DIR}/tcp-stream-${SYS_MEM}-fq.eps"
set yrange [0:]
...
plot \
   ...
   '${OUTPUT}/tcp-stream-native-bbr-fq-${SYS_MEM}.dat' usin (\$0+0.1):(\$1/1000):(\$2/1000) w boxerrorbar fill patter 0 title "Linux(bbr)", \
   '${OUTPUT}/tcp-stream-native-cubic-fq-${SYS_MEM}.dat' usin (\$0+0.3):(\$1/1000):(\$2/1000) w boxerrorbar fill patter 0 title "Linux(cubic)" 

#   '${OUTPUT}/tcp-stream-musl-tap-bbr-fq-${SYS_MEM}.dat' usin (\$0-0.3):1:2 w boxerrorbar fill patter 0 title "LKL(bbr)", \
#   '${OUTPUT}/tcp-stream-musl-tap-cubic-fq-${SYS_MEM}.dat' usin (\$0-0.1):1:2 w boxerrorbar fill patter 0 title "LKL(cubic)" ,\
...
set output "${OUTPUT}/tcp-stream-${SYS_MEM}-fq.png"
replot
...
## set output "${OUTPUT}/${DIR}/tcp-stream-${SYS_MEM}-nofq.eps"
## plot \
...
## 
## set terminal png lw 3 14
## set output "${OUTPUT}/tcp-stream-${SYS_MEM}-nofq.png"
## replot
## 
